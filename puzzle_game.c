...
// Çatışma (Linear Conflict) hesaplama
//taşla birbirini engelliyorsa ekstra ceza puanı üretir
int linear_conflict_hesapla() {
    int ek_puan = 0;

    // Satır bazlı çatışma kontrolü
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            for (int k = j + 1; k < 3; k++) { //aynı satır içinde iki farklı taş
                int tj = tahta[i][j];
                int tk = tahta[i][k];
                
                if (tj != 0 && tk != 0 && (tj-1)/3 == i && (tk-1)/3 == i) {
                    if ((tj-1)%3 > (tk-1)%3) {
                        ek_puan += 2; // Taşlar birbirini engelliyorsa +2 ceza puanı
                    }
                }
            }
        }
    ...
    for (int j = 0; j < 3; j++) {
        for (int i = 0; i < 3; i++) {
            for (int k = i + 1; k < 3; k++) {
                ...
                if (ti != 0 && tk != 0 && (ti-1)%3 == j && (tk-1)%3 == j) {
                    if ((ti-1)/3 > (tk-1)/3) {
                        ek_puan += 2; // Taşlar birbirini engelliyorsa +2 ceza puanı
                    }
                }
...
//  İki skoru birleştiren ana puanlama fonksiyonu
int toplam_puan_hesapla() {
    return manhattan_hesapla() + linear_conflict_hesapla();
}
...
int iki_adim_test(char yon1) { //tek bir yöni alır ve dener "ne kadar iyi?""
    int en_iyi = 9999; //başta büyük çünkü min skoru arıyoruz
    ...
    for(int i=0;i<4;i++) { //şimdi bir adım daha ileri bakıyoruz
        ...
        // KRİTİK DÜZELTME: İkinci hamle, ilk hamlenin tersi olamaz!
        if(ters_mi(yon1, yonler[i])) continue;
        ...
        if(s>=0 && s<3 && t>=0 && t<3) {
            ...
            // DEĞİŞİKLİK: Sadece Manhattan değil, Doğrusal Çatışma da hesaba katılıyor
            int m = toplam_puan_hesapla();
            if(m < en_iyi) en_iyi = m; //en küçük skor = en iyi durum
    ...
    tahta[ys][yt] = tas; //her şey eski haline döndürülür
    tahta[es][et] = 0;
...
void tahtayi_yazdir() {
    ...
    printf("Hamle: %d\n", hamle_sayisi);
    // DEĞİŞİKLİK: Kullanıcıya her iki skoru da gösterelim ki yapay zekanın neye göre karar verdiğini gör
    printf("Kalan Uzaklik (Manhattan + Catisma): %d\n\n", toplam_puan_hesapla());
...
void ipucu_ver() {
    ...
    int en_iyi = 9999; //en küçük skor araniyor
...
int main() {
    ...
    while(1) { //sonsuz oyun döngüsü
        ...
        if(manhattan_hesapla()==0) { //tüm taşlar doğru yerdeyse biter
